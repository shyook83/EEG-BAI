...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 13 14:26:13 2019
...
def build_densenet_forCAM(input_shape, num_outputs, densenettype):
   ...
   # global img_input
    ...
    #x = ZeroPadding3D((2, 2, 2), name='conv1_zeropadding')(img_input)
    
    if densenettype == -2 or densenettype == -1 or densenettype == 0:
        x = Conv3D(nb_filter, (3, 3, 3), strides=(2, 1, 1), name='conv1', use_bias=False)(x)
    else:
        x = Conv3D(nb_filter, (3, 3,3), strides=(2, 2, 2), name='conv1', use_bias=False)(x)
    x = BatchNormalization(epsilon=eps, axis=concat_axis, name='conv1_bn')(x)
    x = Scale(axis=concat_axis, name='conv1_scale')(x)
    x = Activation('relu', name='relu1')(x)
    x = ZeroPadding3D((1, 1, 1), name='pool1_zeropadding')(x)
    if densenettype == -1 or densenettype == 0:
        x = MaxPooling3D((3, 3, 3), strides=(2, 1, 1), name='pool1')(x)
       # x = MaxPooling3D((2, 2, 2), strides=(2, 1, 1), name='pool1')(x)
    
    else:
        x = MaxPooling3D((3, 3, 3), strides=(2, 2, 2), name='pool1')(x)
      #  x = MaxPooling3D((2, 2, 2), strides=(2, 2, 2), name='pool1')(x)
    # Add dense blocks
...
##############################################################################
   ...
   # x = Activation('softmax', name='prob')(x)
    ...
    model = Model(img_input, x, name='densenet2d')
...
def conv_block(x, stage, branch, nb_filter, dropout_rate=None, weight_decay=1e-4):
    ...
    #x = Conv3D(nb_filter, (2, 2, 2), name=conv_name_base+'_x2', use_bias=False)(x)
